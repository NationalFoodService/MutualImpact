...
class ProfileEditForm extends Component {
  ...
  componentDidMount() {
    db.getUser(auth.getCurrentUserId()).then((user) =>
      this.setState({
        user,
        name: user.name,
        email: user.email,
  ...
  onUpdateProfile = (event) => {
    ...
    Promise.all([auth.doUpdateProfile({ name }), auth.doUpdateEmail(email)])
      .then(() => {
        this.setState({
          profileUpdating: false,
          profileSuccess: true,
  ...
  onUploadPhoto = (photo) => {
    this.setState({ photo });
    auth.doUpdateProfile({ photo }).catch((error) => {
      console.log(error);
    });
  };
  ...
  onUpdatePassword = (event) => {
    ...
    auth
      .doUpdatePassword(this.state.password)
      .then(() => {
