...
const objContains = (obj, fields) =>
  fields.every((field) => Object.keys(obj).includes(field));
...
export const sendTokens = async (from, to, details) => {
  if (!objContains(details, ["amount"]))
    throw Error("Sending amount not provided");

  if (from.id === to.id) throw Error("Trying to send to self.");
  if (!isAmountCorrect(details.amount))
...
export const requestTokens = (fromOrg, requester, details) => {
  // validate fromOrg
  if (!isAmountCorrect(details.loggedHours))
    throw Error("Amount of people helped has to be a positive number.");
  if (!isAmountCorrect(details.mealsProvided))
    throw Error("Amount of meals provided has to be a positive number.");
  ...
  const tokenEvent = makeTokenEvent(eventType, {
    fromOrg,
    requester,
    ...details,
  });
...
export const approveTokens = (requestId, orgId) => {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const requesterRef = db
      .collection("users")
      .doc(requestDoc.data().requesterId);
    ...
    const requesterTokens =
      (requesterDoc.data().hours || 0) + Number(requestDoc.data().tokens);
    const requesterMeals =
      (requesterDoc.data().meals || 0) + Number(requestDoc.data().meals);
    // Note: Hours is actually the value for number of people helped
    const distributedHours =
      (orgDoc.data().hoursGenerated || 0) + Number(requestDoc.data().tokens);
    const distributedMeals =
      (orgDoc.data().mealsProvided || 0) + Number(requestDoc.data().meals);
...
export const rejectTokens = (request, orgId, reason) => {
  if (request.fulfilled)
    return Promise.reject(Error("Request already fulfilled"));
